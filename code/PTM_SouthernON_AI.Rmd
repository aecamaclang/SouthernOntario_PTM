...
author: "AranyaIyer"
date: "2023-03-10" ####LAST UPDATED  -> 2025-07-09 by AC: updated folder references; added complementarity analysis for Benefits data
output: html_document
---

This is the document to be used for PTM Southern Ontario analysis. It has all the separate codes in one Markdown file to streamline for efficiency. Only the final codes should be put and updated here. 

Scripts are originally written by Abbey C. and are then modified by Aranya I. The original scripts can be pulled from https://github.com/aecamaclang/PTM_3-point-est
...
input <- here("data", "raw") # where input data files are stored (benefit estimates, cost & feasibility table, etc.)
derived <- here("data") # where compiled data tables should be saved
...
# NOTE that these files are not included in the archive to maintain expert confidentiality; instead an empty template file is provided

# read in the files 
files <- list.files(path = paste(input, "/benefits/", sep=""),
                    pattern = "*.csv", 
                    full.names = T)
...
# Standardize column/row names for the Southern Ontario PTM project 
# Must match the names in SpecialCases.csv and EcolGroupsList.csv
data$Ecological.Group[which(str_detect(data$Ecological.Group, "Alvar/Rock barren/Shield") == 1)] <- "Alvar species"
data$Ecological.Group[which(str_detect(data$Ecological.Group, "Artificial/Man-made Structures") == 1)] <- "Artificial structure dependent spp"
...
data$Ecological.Group[which(str_detect(data$Ecological.Group, "Naturalized Open habitats") == 1)] <- "Naturalized open habitat spp"
...
write_csv(long, file = paste(derived, "/Estimates_tidy.csv", sep = "")) #Should have 9984 obs for 5 variables (16 species groups * 3 estimates * 13 experts = 624*16 (including baseline)=9984)
# file excluded from archive
...
write_csv(wide, file = paste(derived, "/Estimates_wide.csv", sep = "")) #Should have 131 obs for 50 variables
# file excluded from archive
...
stgy.count <- table(temp$Ecological.Group, temp$Strategy) #should roughly align with your CostFeasCOUNTS_ON

write.csv(stgy.count, file = paste(results, "/estimatecounts.csv", sep = ""))
```

Step 1A: Plot the imported data [ONLY NEEDED IF CREATING SUMMARY PLOTS FOR EXPERT REVIEW]
...
derived <- here("data") # where compiled data tables are saved
results <- here("results") # where results of analysis should be saved
figures <- here("figures") # where plots should be saved 
# NOTE plots showing individual expert estimates have been excluded from the archived version
...
If some of the expert estimates for a given ecological group are based on only a subset of species in that group, this estimate need to be weighted accordingly. To do this, you need to create:  
...
Names of ecological groups in both files must match the names used in the compiled benefit estimates tables. Store both files in the ~/data/raw folder.
...
# Specify paths to subfolders within current working directory
input <- here("data", "raw") # where unchanging data are 
derived <- here("data") # where compiled data tables should be saved
results <- here("results") # where results of analysis should be saved
...
long <- read_csv(paste0(derived, "/Estimates_tidy.csv")) ##Should be 9984 obs for 5 variables 
grp.levels <- unique(long$Ecological.Group)
...
spcases <- read_csv(paste0(input, "/SpecialCases.csv")) # NOTE this dataset is manually compiled from the individual expert tables
...
# Aggregate (sum) the weighted estimates and re-organize table for calculating performance (i.e.,average benefit for species group across all experts)
benefit.avg <- aggregate(benefit.joined[,11:13], 
  ...
  gather(., key = "Est.Type", 
         value = "Wt.Avg", 
         Wt.Best, Wt.Low, Wt.High) #Should be 720 rows because 16 species groups * 15 strategies *3 estimates = 720
...
#' Weight benefits by number of species in group (multiply) for calculating CE scores if you did the weighted species 
...
Step 2A: Plot the aggregated/averaged values [ONLY NEEDED IF CREATING SUMMARY PLOTS FOR EXPERT REVIEW, OR IF DESIRED FOR FINAL REPORT/MANUSCRIPT]
...
temp.plot2 <- 
  ...
  theme(plot.margin = unit(c(1.5, 1, 1.5, 1), "cm"), # top, right, bottom and left margins around the plot area
        ...
        axis.text.x = element_text(size = 8, angle = 65
                                   , hjust = 1, vjust = 1)
        ,
        plot.caption = element_text(size =10, hjust = 0)
...
# ggsave(filename=paste0(figures, est.file, "_plot.png"), 
#        temp.plot2, 
#        width = 8.5, height = 11, 
#        units = "in")
...
names(costfeas)[2] <- "Cost" # col 2 is the cost with 3% PV discount rate 
names(costfeas)[3] <- "Avg.Feas" 
...
bestben <- ben.grpwtd[,-1] %>%
  ...
  filter(grepl("Best", rownames(.))) #Focuses only on the best benefits at this stage. 

bestben.sum <- data.frame(rowSums(bestben)) %>% #takes the sum of all the rows (ie., sp groups) and creates a benefit value PER strategy
...
#' Cost-effectiveness = (Benefit * Feasibility)/Cost
CE_table <- full_join(bestben.sum, costfeas, by="Strategy")%>%
  mutate(Exp.Benefit = Benefit * Avg.Feas) %>% # weight benefits by feasibility
...
#, Min.ExpBen = Value * min.Feas) # removing this because we do not work with min.Feas
...
write.csv(exp.ben, paste0(results, "/ExpBenefits.csv"), row.names = FALSE) #NOTE: This is not just benefit. It is benefit and feasibility combined.  
...
This code performs the complementarity analysis using the consOpt R package developed by Nicolai Cryer <nkcryer@gmail.com> and updated by Abbey Camaclang with a number of bug fixes. Note that this package is still under development; we recommend checking the results manually.
...
Requires **ExpPerform_all.csv** from previous step, and the **CostFeas.csv** table of cost and feasibility for each strategy. Also requires a table **Combinations.csv** specifying which individual strategies are in the combination strategies.
...
exp.perf <- read.csv(paste0(results, "/ExpPerform_all.csv")) #from previous step 
costfeas <- read.csv(paste0(input, "/CostFeas.csv")) 
...
results.best <- Optimize(benefits.matrix = best, 
                    ...
                    , thresholds = c(50.00, 60.00) #do .00 not .01. #can put 2-3 thresholds 
                    )
...
cols <- c("50" = "goldenrod1", "60" = "darkolivegreen3", "70" = "darkmagenta")
...
PlotOptCurve <- function(summary.results, benefits.matrix, draw.labels=TRUE){
  ...
  this.plot <- ggplot(tmp, aes(
    ...
    theme(legend.justification = c(1,0),
          ...
          # legend.position = c(0.95,0.5), # use to plot pessimistic/lowest estimates instead of previous line
...
This code performs the complementarity analysis as above, using estimates of the expected benefit (Benefit * Feasibility) instead of the expected probability of persistence.  

Requires **ExpBenefits.csv** from Step 4, and the **CostFeas.csv** table of cost and feasibility for each strategy. Also requires a table **Combinations.csv** specifying which individual strategies are in the combination strategies.
...
input <- here("data", "raw") # where raw data files are located
results <- here("results") # where results of analysis should be saved
results_ben <- here("results") # where results of benefits analysis should be saved
figures <- here("figures") # where plots should be saved

combos <- read.csv(paste0(input, "/Combinations.csv"), header = TRUE) # list of individual strategies that make up each strategy (in columns). Should have a column for baseline and all strategies
exp.ben <- read.csv(paste0(results, "/ExpBenefits.csv")) 
costfeas <- read.csv(paste0(input, "/CostFeas.csv")) # estimated Cost and Feasibility for each Strategy (col 1), including Baseline
costs <- costfeas$Cost.3disc  
names(costs) <- costfeas$Strategy
...
PlotOptCurve <- function(summary.results, benefits.matrix, draw.labels=TRUE){
  ...
  this.plot <- ggplot(tmp, aes(
    ...
    labs(x = "Total cost (million CAD)", 
         y = "Number of species groups benefitting"
         , color="Expected increase in\nprobability of persistence (%)"
         # , linetype = Expected increase in\nprobability of persistence (%)"
         , shape = "Expected increase in\nprobability of persistence (%)"
         )
...
#' Create matrix for complementarity analysis (optimization)
...
#' Complementarity analysis for most likely (best guess) estimates
results.bestben <- Optimize(benefits.matrix = best.ben, 
                    cost.vector = costs, 
                    combo.strategies = combos
                    , thresholds = c(15.00) #can change thresholds here
                    )
...
optcurve.bestben <- PlotOptCurve(results.bestben, best.ben, draw.labels = TRUE)

ggsave(paste0(figures, "/Complementarity_best_Benefits.pdf"), optcurve.bestben, width = 180, height = 120, units = "mm")


#' Uncertainty analysis
# Create matrices for lowest and highest estimates
low.ben <- ben.transposed %>%
  filter(grepl("Low", Estimate)) %>%
  mutate(Estimate = NULL) %>%
  column_to_rownames("Strategy")

low.ben <- round(low.ben, digits = 0)

high.ben <- ben.transposed %>%
  filter(grepl("High", Estimate)) %>%
  mutate(Estimate = NULL) %>%
  column_to_rownames("Strategy")

high.ben <- round(high.ben, digits = 0)

write.csv(low.ben, paste0(results, "/ExpBenefits_low.csv"), row.names = FALSE) 
write.csv(high.ben, paste0(results, "/ExpBenefits_high.csv"), row.names = FALSE) 

# Run complementarity analysis for the most pessimistic (lowest estimates) and most optimistic (highest estimates)
results.lowben <- Optimize(benefits.matrix = low.ben,
                  cost.vector = costs,
                  combo.strategies = combos
                  , thresholds = c(10.00, 15.00, 20.00)
                  )

results.highben <- Optimize(benefits.matrix = high.ben,
                  cost.vector = costs,
                  combo.strategies = combos
                  , thresholds = c(10.00, 15.00, 20.00)
                  ) 

write.csv(results.lowben, paste0(results, "/Complementarity_low_Benefits.csv"), row.names = FALSE)
write.csv(results.highben, paste0(results, "/Complementarity_high_Benefits.csv"), row.names = FALSE)


# Plot results using the custom plot function above:
optcurve.lowben <- PlotOptCurve(results.lowben, low.ben, draw.labels = TRUE)
optcurve.highben <- PlotOptCurve(results.highben, high.ben, draw.labels = TRUE)

ggsave(paste0(figures, "/Complementarity_low_Benefits.png"), optcurve.lowben, width = 180, height = 120, units = "mm")
ggsave(paste0(figures, "/Complementarity_high_Benefits.png"), optcurve.highben, width = 180, height = 120, units = "mm")

```

Step 6: Uncertainty analysis (Apr-13) #Delete? or update
 ...
 Needs:  
 1) **Estimates_tidy.csv** from Step 1,  
 ...
 is based on,  
 4) **Benefits_total.rds** from Step 3, and  
...
# Specify paths to subfolders within current working directory
input <- here("data", "raw") # where .csv files of benefit estimates are saved
derived <- here("data") # where compiled data tables should be saved
results <- here("results") # where results of analysis should be saved
...
names(costfeas)[2] <- "Cost" # 3% discount
names(costfeas)[3] <- "Avg.Feas" #Average feasibility only 
costfeas$Strategy <- as_factor(costfeas$Strategy)
costfeas <- costfeas[-1,] # Remove baseline values
